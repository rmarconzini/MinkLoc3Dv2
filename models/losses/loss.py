...
from models.losses.der_triplet import DerTripletMarginLoss
...
def make_losses(params: TrainingParams):
    if params.loss == "batchhardtripletmarginloss":
        ...
        loss_fn = BatchHardContrastiveLossWithMasks(
            params.pos_margin, params.neg_margin
        )
    elif params.loss == "truncatedsmoothap":
        loss_fn = TruncatedSmoothAP(
            tau1=params.tau1,
...
class HardTripletMinerWithMasks:
    ...
    def mine(self, embeddings, positives_mask, negatives_mask):
        ...
        a = torch.arange(dist_mat.size(0)).to(hardest_positive_indices.device)[
            a_keep_idx
        ]
...
class BatchHardContrastiveLossWithMasks:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        stats = {
            "loss": loss.item(),
            "avg_embedding_norm": self.loss_fn.distance.final_avg_query_norm,
            "pos_pairs_above_threshold": self.loss_fn.reducer.reducers[
                "pos_loss"
            ].pos_pairs_above_threshold,
            "neg_pairs_above_threshold": self.loss_fn.reducer.reducers[
                "neg_loss"
...
class DerBasedBatchHardTripletLossWithMask:
    def __init__(self, margin: float, der_lambda: float = 0.01):
        self.margin = margin
        self.der_lambda = der_lambda
        self.distance = LpDistance(normalize_embeddings=False, collect_stats=True)
        # We use triplet loss with Euclidean distance
        self.miner_fn = HardTripletMinerWithMasks(distance=self.distance)
        self.reducer_fn = reducers.AvgNonZeroReducer(collect_stats=True)
        self.loss_fn = DerTripletMarginLoss(
            margin=self.margin,
            swap=True,
            distance=self.distance,
            collect_stats=True,
    ...
    def __call__(self, embeddings_dict, positives_mask, negatives_mask):
        # Extract components from embeddings_dict
        global_embeddings = embeddings_dict["global"]
        gamma = embeddings_dict["gamma"]
        nu = embeddings_dict["nu"]
        alpha = embeddings_dict["alpha"]
        beta = embeddings_dict["beta"]

        hard_triplets = self.miner_fn(global_embeddings, positives_mask, negatives_mask)
        dummy_labels = torch.arange(global_embeddings.shape[0]).to(
